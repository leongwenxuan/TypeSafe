"""
OpenAI integration for scam text analysis.
"""
import asyncio
import json
import logging
...
def get_client() -> AsyncOpenAI:
    ...
    if _client is None:
        ...
        _client = AsyncOpenAI(
            ...
            timeout=1.5  # 1.5s timeout for OpenAI API calls
        )
...
def _normalize_response(raw_response: str) -> Dict[str, Any]:
    """
    Parse and normalize OpenAI response to unified schema.
...
async def analyze_text(text: str) -> Dict[str, Any]:
    """
    Analyze text for scam intent using OpenAI.
    ...
    try:
        ...
        logger.warning(f"OpenAI API timeout for text length {len(text)}")
